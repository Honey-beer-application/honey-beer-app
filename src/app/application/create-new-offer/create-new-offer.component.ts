...
export class CreateNewOfferComponent implements OnDestroy {
  ...
  private readonly productsObservable:BehaviorSubject<IProduct[]> = new BehaviorSubject(<IProduct[]>[]);
  ...
  constructor(private readonly productController:ProductController,private readonly fb:FormBuilder){
    ...
    this.subs.add(
      this.productController.loadAllProducts().subscribe(
        {
          next: (data:IProduct[])=>{this.productsObservable.next(data);this.products=data},
